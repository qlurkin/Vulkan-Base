...
void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, Engine* engine) {
	...
	if (vkCreateImage(engine->getDevice(), &imageInfo, nullptr, &image) != VK_SUCCESS) {
		throw std::runtime_error("failed to create image!");
	}

	VkMemoryRequirements memRequirements;
	vkGetImageMemoryRequirements(engine->getDevice(), image, &memRequirements);
	...
	if (vkAllocateMemory(engine->getDevice(), &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
		throw std::runtime_error("failed to allocate image memory!");
	}

	vkBindImageMemory(engine->getDevice(), image, imageMemory, 0);
}
...
VkImageView createImageView(VkImage image, VkFormat format, Engine* engine) {
	...
	if (vkCreateImageView(engine->getDevice(), &viewInfo, nullptr, &imageView) != VK_SUCCESS) {

...
GraphicsPipeline::GraphicsPipeline(VertexInputState vertexInputState, Shader& vertexShader, Shader& fragmentShader, DescriptorSetLayout& descriptorSetLayout, Engine* engine) : engine(engine) {
	...
	VkPipelineVertexInputStateCreateInfo vertexInputInfo = vertexInputState;
	...
	if (vkCreateGraphicsPipelines(engine->getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
		throw std::runtime_error("failed to create graphics pipeline!");
	}

	LOG << "GraphicsPipeline Created" << ENDL; 
}
